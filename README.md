...
Освоение базовых принципов параллельного программирования в модели передачи сообщений (MPI). Разработка параллельной реализации умножения квадратных матриц, проведение серии вычислительных экспериментов и оценка эффективности разработанного решения.
...
2. Получение общего количества процессов и идентификатора каждого процесса
3. Распределение строк исходной матрицы между вычислительными процессами
4. Вычисление каждым процессом своей части результирующей матрицы
5. Синхронизация процессов с помощью барьера (MPI_Barrier)
6. Измерение времени выполнения с использованием std::chrono
7. Трёхкратный повтор каждого вычислительного эксперимента
8. Сохранение полученных результатов в CSV-файл
9. Генерация итогового отчёта по результатам измерений
...
| ------ | --------- | ---------- | ---------- |
| 200    | 0.0936    | 0.0469     | 0.0242     |
| 400    | 0.6574    | 0.3493     | 0.1663     |
| 800    | 4.7675    | 2.496     | 1.3026     |
| 1200   | 16.105    | 8.1741     | 4.3259     |
| 1600   | 37.518    | 18.8562    | 9.7496     |
| 2000   | 72.932    | 36.6453    | 18.7933    |
...
* Увеличение числа задействованных процессов ведёт к пропорциональному сокращению времени счёта
* Наблюдаемое ускорение близко к идеальному (линейному)
* Эффективность параллелизации возрастает с увеличением размерности матриц
* На малых размерах матриц заметно влияние коммуникационных накладных расходов
* Применённая схема распределения строк обеспечивает сбалансированную загрузку всех процессов
  
  Время в зависимости от процесса
<img width="1204" height="719" alt="image" src="https://github.com/user-attachments/assets/eb2df05b-4946-4836-a2db-608f0ff47acf" />


Ускорение
<img width="1240" height="754" alt="image" src="https://github.com/user-attachments/assets/40d9982d-b56c-4b98-9331-a7c6334e703f" />
...
1. Разработан параллельный алгоритм умножения матриц на основе технологии MPI.
2. Проведена серия экспериментов для матриц различной размерности.
3. Зафиксировано ускорение до 3.95 раз при использовании 4 процессов.
